...
export class Querier {
  ...
  public async player(player: string): Promise<Player | null> {
    ...
    return {
      ...
      nationality: info.nationality?.value ?? null,
      assists: info.assists?.value ?? null,
      height: info.height?.value ?? null,
      weight: info.weight?.value ?? null,
      transactionFee: info.transactionFee?.value ?? null,
      age: info.age?.value ?? null,
      titles: d2.results.bindings.map((item) => ({
        title: item.title.value,
        titleName: item.titleName.value,
      })),
  ...
  public async team(team: string): Promise<Team | null> {
    ...
    const q2 = footballTeamLeagueSeasonQuery.replaceAll(
      /{footballTeam}/g,
      team
    );
    ...
    return {
      id: team,
      des: info.des?.value ?? null,
      teamLogo: info.teamLogo?.value ?? null,
      image: info.image?.value ?? null,
      name: info.name.value,
      ...
      League: d2.results.bindings.map((item) => ({
        ls: item.ls.value,
        lsName: item.lsName.value,
      })),
      Match: d3.results.bindings.map((item) => ({
        ms: item.ms.value,
        msName: item.msName.value,
      })),
    };
  }
  ...
  public async league(league: string): Promise<League | null> {
    ...
    return {
      ...
      Teams: d2.results.bindings.map((item) => ({
        fbTeam: item.fbTeam.value,
        fbTeamName: item.fbTeamName.value,
      })),
      Match: d3.results.bindings.map((item) => ({
        match: item.match.value,
        matchName: item.matchName.value,
      })),
    };
  }
